...
export const SegmentedGroup: FunctionComponent<SegmentedGroupProps> = props => {
  ...
  const varClasses = useMemo(() => {
    return joinTrim([nsg.b(), props.className]);
  }, []);
  ...
  return (
    <SegmentedContext.Provider
      value={{ parent: { props: { ...props } }, toggle: onToggle, checked: value }}
    >
      <div className={varClasses} style={varStyles}>
        <div
          className={ns.m('selected')}
          style={{
            left: 186,
            ...varSelectedStyles,
          }}
        ...
        {React.Children.map(props.children, (item, index) => {
          if (item) {
            return React.cloneElement(
              item as React.ReactElement<any, string | React.JSXElementConstructor<any>>,
              {
                index,
              }
            );
