...
export const Skeleton: FunctionComponent<SkeletonProps> = props => {
  ...
  const varClasses = useMemo(() => {
    return joinTrim([ns.b(), animate ? ns.m('animate') : '', props.className]);
  }, []);
  ...
  function renderAvatar() {
    if (props.avatar) {
      return (
        <div
          className={joinTrim([ns.e('avatar'), ns.m(avatarShape)])}
  ...
  function renderTitle() {
    if (props.title) {
      ...
      return (
        <div
          className={joinTrim([ns.e('title'), round ? ns.m('round') : ns.m('square')])}
          style={{ width, height }}
        />
      );
    }
    ...
    const rowChildren = Array(row)
      ...
      .map((_, i) => {
        ...
        return (
          <div
            key={i}
            className={joinTrim([ns.e('row'), round ? ns.m('round') : ns.m('square')])}
            style={{ width, height }}
          />
        );
      });
